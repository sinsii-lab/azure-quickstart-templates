...
%macro csbmva_accura(input_ds=,output_ds=,flag=);
...
name=put(time(),mmss8.1);/*i18NOK:LINE */

name1=kscan(name,1,":.");/*i18NOK:LINE */

name2=kscan(name,2,":.");/*i18NOK:LINE */

name3=kscan(name,3,":.");/*i18NOK:LINE */
...
Predicted_event = &FP;
Actual = "GOOD";/*i18NOK:LINE *//* The value GOOD is read as a code in midtier and a resource bundle key is maintained to resolve the locale specific value*/
...
Predicted_event = &TP;
Actual = "BAD";/*i18NOK:LINE *//* The value BAD is read as a code in midtier and a resource bundle key is maintained to resolve the locale specific value*/
total = Predicted_non_event+Predicted_event;
OUTPUT;
END;
END;
...
values ("TOTAL",&a,&b,&c);/*i18NOK:LINE *//* The value TOTAL is read as a code in midtier and a resource bundle key is maintained to resolve the locale specific value*/
quit;
